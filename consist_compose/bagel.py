...
from .utils import add_special_tokens, resize_image_to_short_edge, draw_bbox, extract_bbox_and_subject
import random
import numpy as np
from PIL import Image
...
BASE_PARAMS: Dict[str, Dict[str, Any]] = {
    "generate": dict(
        cfg_text_scale=4.0, cfg_img_scale=1.0, cfg_coord_scale=None,
        cfg_interval=[0.4, 1.0], timestep_shift=3.0, num_timesteps=50,
        cfg_renorm_min=0.0, cfg_renorm_type="global",
    ),
    "think_generate": dict(
        max_think_token_n=1000, do_sample=False, cfg_text_scale=4.0, cfg_coord_scale=None,
        cfg_img_scale=1.0, cfg_interval=[0.4, 1.0], timestep_shift=3.0,
        num_timesteps=50, cfg_renorm_min=1.0, cfg_renorm_type="global", think=True,
    ),
    "edit": dict(
        cfg_text_scale=4.0, cfg_img_scale=2.0, cfg_coord_scale=None,
        cfg_interval=[0.0, 1.0], timestep_shift=3.0, num_timesteps=50,
        cfg_renorm_min=0.0, cfg_renorm_type="text_channel",
    ),
    "think_edit": dict(
        max_think_token_n=1000, do_sample=False, cfg_text_scale=4.0, cfg_coord_scale=None,
        cfg_img_scale=2.0, cfg_interval=[0.4, 1.0], timestep_shift=3.0,
        num_timesteps=50, cfg_renorm_min=0.0, cfg_renorm_type="text_channel", think=True,
    ),
    "understanding": dict(
        max_think_token_n=1000, do_sample=False, understanding_output=True,
    ),
    "layout_t2i": dict(
        cfg_text_scale=4.0, cfg_img_scale=1.0, cfg_coord_scale=1.6,
        cfg_interval=[0.4, 1.0], timestep_shift=3.0, num_timesteps=50,
        cfg_renorm_min=0.0, cfg_renorm_type="global",
    ),
    "layout_subject_driven": dict(
        cfg_text_scale=4.0, cfg_img_scale=2.0, cfg_coord_scale=0.6,
        cfg_interval=[0.0, 1.0], timestep_shift=3.0, num_timesteps=50,
        cfg_renorm_min=0.0, cfg_renorm_type="text_channel", image_shapes=(512, 512)
    ),
}
...
class ConsistComposeBagelModel(Model):
    def __init__(
        self,
        ...
        if local_model_path.exists() and local_model_path.is_dir():
            ...
            print(f"[ConsistCompose] Using local model path: {self.model_path}")
        else:
            print(f"[ConsistCompose] Local path {model_path} does not exist, starting to download model {model_path} from Hugging Face...")
            try:
                cache_path = snapshot_download(repo_id=model_path)
                self.model_path = cache_path
                print(f"[ConsistCompose] Model download completed, cache path: {self.model_path}")
            except Exception as e:
                raise RuntimeError(f"Failed to download model! Reason: {str(e)}\nPlease check network connection or confirm model_path is correct") from e
        ...
        msg = (
            f"[Bagel] out_img_dir = '{self.out_img_dir}'; "
            f"mode will be specified in generate() method"
        )
        print(msg)
    ...
    def _save_output_image(
        ...
        elif mode in ["generate", "think_generate", "layout_t2i", "layout_subject_driven"]:
            out_dir = images_root
            out_dir.mkdir(parents=True, exist_ok=True)
    ...
    def generate(self, question: str, images: list[str] | None = None, mode: str = None, vis_bbox=False, resize_short_edge: int = 512, **kwargs):
        ...
        if len(text_parts) != len(images) + 1:
            raise ValueError(f"Text image tokens and number of images not match! ")
        ...
        if mode in ["edit", "think_edit", "generate", "think_generate", "layout_t2i", "layout_subject_driven"]:
            if ret["image"] is not None:
                ...
                if vis_bbox and mode in ["layout_t2i", "layout_subject_driven"]:
                    pure_prompt = text_parts[-1]
                    bbox_info = extract_bbox_and_subject(pure_prompt)
                    ret["image"] =  draw_bbox(ret["image"], bbox_info)
