...
class Bagel(PreTrainedModel):
    ...
    def generate_image(
        ...
        #cfg_coord
        cfg_coord_scale=None,
        cfg_coord_packed_query_indexes: Optional[torch.LongTensor] = None,
        cfg_coord_packed_position_ids: Optional[torch.LongTensor] = None,
        cfg_coord_past_key_values: Optional[NaiveCache] = None,
        cfg_coord_key_values_lens: Optional[torch.IntTensor] = None,
        cfg_coord_packed_key_value_indexes: Optional[torch.LongTensor] = None,
        ...
        for i, t in tqdm(enumerate(timesteps), total=len(timesteps)):
            ...
            if t > cfg_interval[0] and t <= cfg_interval[1]:
                ...
                cfg_coord_scale_ = cfg_coord_scale
            else:
                cfg_text_scale_ = 1.0
                cfg_img_scale_ = 1.0
                cfg_coord_scale_= None
            v_t = self._forward_flow(
                ...
                #cfg_coord
                cfg_coord_scale=cfg_coord_scale_,
                cfg_coord_packed_query_indexes=cfg_coord_packed_query_indexes,
                cfg_coord_packed_position_ids=cfg_coord_packed_position_ids,
                cfg_coord_past_key_values=cfg_coord_past_key_values,
                cfg_coord_key_values_lens=cfg_coord_key_values_lens,
                cfg_coord_packed_key_value_indexes=cfg_coord_packed_key_value_indexes,
                cfg_type=cfg_type,
            )
    ...
    def _forward_flow(
        ...
        # if cfg_coord_scale > 1.0:
        if cfg_coord_scale is not None:
            cfg_coord_output = self.language_model.forward_inference(
                packed_query_sequence=packed_sequence,
                query_lens=packed_seqlens,
                packed_query_position_ids=cfg_coord_packed_position_ids,
                packed_query_indexes=cfg_coord_packed_query_indexes,
                past_key_values=cfg_coord_past_key_values,
                key_values_lens=cfg_coord_key_values_lens,
                packed_key_value_indexes=cfg_coord_packed_key_value_indexes,
                update_past_key_values=False,
                is_causal=False,
                **extra_inputs,
            )
            cfg_coord_v_t = self.llm2vae(cfg_coord_output.packed_query_sequence)
            cfg_coord_v_t = cfg_coord_v_t[packed_vae_token_indexes]
        ...
        if cfg_text_scale > 1.0:
            if cfg_renorm_type == "text_channel":
                ...
                # # if cfg_coord_scale is not None:
                # #     v_t = cfg_coord_v_t + cfg_coord_scale * (v_t_img - cfg_coord_v_t)
                # if cfg_coord_scale is not None:
                #     v_t_coord_ = cfg_coord_v_t + cfg_coord_scale * (v_t_img - cfg_coord_v_t)
                #     norm_v_t = torch.norm(v_t, dim=-1, keepdim=True)
                #     norm_v_t_coord_ = torch.norm(v_t_coord_, dim=-1, keepdim=True)
                #     scale = (norm_v_t / (norm_v_t_coord_ + 1e-8)).clamp(min=cfg_renorm_min, max=1.0)
                #     v_t_coord = v_t_coord_ * scale
                v_t_text_ = cfg_text_v_t + cfg_text_scale * (v_t - cfg_text_v_t)
                norm_v_t = torch.norm(v_t, dim=-1, keepdim=True)
                norm_v_t_text_ = torch.norm(v_t_text_, dim=-1, keepdim=True)
                scale = (norm_v_t / (norm_v_t_text_ + 1e-8)).clamp(min=cfg_renorm_min, max=1.0)
                ...
                if cfg_coord_scale is not None:
                    v_t_coord_ = cfg_coord_v_t + cfg_coord_scale * (v_t_text - cfg_coord_v_t)
                    norm_v_t = torch.norm(v_t, dim=-1, keepdim=True)
                    norm_v_t_coord_ = torch.norm(v_t_coord_, dim=-1, keepdim=True)
                    scale = (norm_v_t / (norm_v_t_coord_ + 1e-8)).clamp(min=cfg_renorm_min, max=1.0)
                    v_t_coord = v_t_coord_ * scale
                ...
                if cfg_img_scale > 1.0:
                    v_t = cfg_img_v_t + cfg_img_scale * (v_t_coord - cfg_img_v_t)
                    v_t_img = v_t
                else:
                    v_t = v_t_coord
                    v_t_img = v_t
                ...
                if cfg_coord_scale is not None:
                    v_t_ = cfg_coord_v_t + cfg_coord_scale * (v_t_img_ - cfg_coord_v_t)
