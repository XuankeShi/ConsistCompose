...
def load_image(image_path, aoss_client=None):
    """ Load image from local or s3, return PIL Image object """
    if "s3://" in image_path:
        assert aoss_client is not None, "aoss_client is required when loading images from s3"
        return load_aoss_image(aoss_client, image_path)
    else:
        return Image.open(image_path)


def load_aoss_image(client, img_url):
